...
- **AC**:
  - [ ] 原子组件是主 API，消费者可仅凭原子组件自行组合筛选器（字段选择、操作符选择、值编辑、组合器、规则行、分组块等）
  - [ ] 可选导出完整 `FilterBuilder` 组合件作为便利层，其编排逻辑集中在 `@x-filter/react` 的 headless 编排器中（见 ADR-0002）
  - [ ] 两个包组件能力一一对应
  - [ ] 样式采用默认 shadcn/antd 风格；具体视觉由业务侧调整
  - [ ] `@x-filter/shadcn` 要求 Tailwind + shadcn 运行环境（peerDependencies）
- **Priority**: P0

> **勘误（2026-06-20）**：原 AC 第一条"仅导出原子组件，不导出官方高阶组合组件"已被 2026-05-28 设计文档和 ADR-0002 修正。原子组件仍是主 API，但 `FilterBuilder` 组合件作为可选便利层导出，其编排逻辑提升到 `@x-filter/react` 以消除 shadcn/antd 重复。`@x-filter/react` 的"无 UI"约束澄清为"无样式化组件 / 无 UI 库耦合"，允许渲染原生 HTML 元素作为无样式语义结构。
