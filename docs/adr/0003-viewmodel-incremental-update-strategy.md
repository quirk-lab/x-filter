...
`packages/core/src/mutations.ts` (`updateRule`, `updateGroup`, `removeRule`,
`moveRule`, …), built on the shared `mapTree` / `updateById` primitives
(see ADR 0001), **already perform structural sharing** — they return the
input subtree unchanged when it is untouched, so unchanged `FilterRule` /
`FilterGroup` references retain `===` identity across a mutation. The
ViewModel layer was discarding this already-paid-for identity.
