...
- [x] #26 JsonLogic/MongoDB/ES 导出 → 新增 3 个 core 编译器（`toJsonLogic`/`toMongoQuery`/`toElasticQuery`），与现有 `toSQL` 同构（递归 group/rule、操作符映射表、`fieldMap` 选项、`not` 取反、空分组跳过、单子节点不加包裹、IC 自动转标准）。每种格式独立操作符映射：JsonLogic（`==`/`in`/链式 `<=` between/`!` 取反，空→`true`）、MongoDB（`$eq`/`$regex` 转义/`$gte+$lte` between/`$nor` 取反，空→`{}`）、Elasticsearch（`term`/`terms`/`wildcard`+`prefix`/`range`/`bool.must|should|must_not`/`exists`，空→`match_all`）。主入口 + 子路径（`@x-filter/core/{jsonlogic,mongodb,elasticsearch}`）双导出 + tsup 入口。新增 52 个单测（每格式覆盖全操作符 + 嵌套 + NOT + IC + fieldMap + 非法操作符）+ README 导出格式章节。验证：core 626 测试全过、typecheck、biome、build
- [x] #27 Responsive 移动端适配 → 新增 headless `useIsMobile(breakpoint=640)`（matchMedia 驱动，SSR/jsdom 无 matchMedia 时默认桌面，故存量测试零改动）从 @x-filter/react 导出。shadcn 用 Tailwind 响应式类（`flex-col items-stretch sm:flex-row sm:flex-wrap`）让规则在窄屏纵向堆叠铺满整行；antd 用 `useIsMobile` 切换 `Space direction=vertical` + 全宽。DnD 两端补齐 `Mouse`/`Touch`(长按 200ms 延迟)/`Keyboard` 三 sensor，拖拽手柄 44px 触摸目标 + `touch-action:none`。新增 react hook 单测（5）+ shadcn/antd 响应式集成测试（4）+ Storybook `Scenarios/Responsive`（手机/桌面对比框）+ README 章节。验证：1051 测试全过、typecheck、biome
- [x] #29 Clear All + 空状态引导 → 新增 headless `clear()`（`useFilterBuilder` 返回 + 经 orchestrator 暴露为 `actions.clear`）：清空根节点 children，保留 `id`/组合子/IC 结构，`locked` 根节点不受影响。新增 6 个 i18n key（`clearAll`/`clearAllConfirm`/`clearAllCancel`/`emptyStateTitle`/`emptyStateDescription`/`emptyStateAction`）写入 `DEFAULT_LABELS` + en/zh/ja locale。shadcn/antd 两端：工具栏 **Clear all** 按钮（两步确认 confirm/cancel，无 children 时禁用）+ 空状态引导卡片（CTA「添加第一条规则」调用 `addRule(root)`）；`readOnly` 下两者均隐藏。新增 react `clear` 单测（3）+ shadcn/antd clear-all 集成测试（各 5）+ 更新 render-decisions 默认 label 断言 + Storybook `Scenarios/Clear All`（shadcn/antd 对比）+ README 章节。验证：1064 测试全过、typecheck、biome
