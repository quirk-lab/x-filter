...
function mapGroupLike(
  ...
  const newConditions = afterOnGroup.children.map((c) => {
  ...
  return { ...afterOnGroup, children: newConditions };
}
...
function removeDuplicateRule(
  ...
  const newConditions: FilterGroup['children'] = [];
  ...
  return changed ? { ...root, children: newConditions } : root;
}
    ...
    onGroup: (g) => {
      ...
      if (g.id === targetGroupId) {
        ...
        if (isSameGroup) {
          ...
          const newConditions = [...group.children];
          ...
          newConditions.splice(position, 0, rule);
          return { ...group, children: newConditions };
        }
...
```ts
  const result = mapTree(filter, {
    onGroup: (g) => {
      ...
      const newConditions: FilterGroupIC['children'] =
