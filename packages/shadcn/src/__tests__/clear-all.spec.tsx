...
describe('ShadcnFilterBuilder clear all', () => {
  test('clear all requires a confirm step before emptying the filter', () => {
    const onChange = jest.fn();
    render(<ShadcnFilterBuilder schema={schema} defaultValue={withRules} onChange={onChange} />);
  ...
  test('clear all is disabled when there is nothing to clear', () => {
    render(<ShadcnFilterBuilder schema={schema} defaultValue={empty} />);
  ...
  test('shows an empty-state guide whose CTA adds the first rule', () => {
    const onChange = jest.fn();
    render(<ShadcnFilterBuilder schema={schema} defaultValue={empty} onChange={onChange} />);
  ...
  test('read-only hides the clear toolbar and empty-state guide', () => {
    render(<ShadcnFilterBuilder readOnly schema={schema} defaultValue={empty} />);
