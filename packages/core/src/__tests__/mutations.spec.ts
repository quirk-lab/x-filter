...
describe('addRule', () => {
  ...
  it('adds rule to nested group', () => {
    ...
    expect(g1).toHaveProperty('children');
    if ('children' in g1) {
      expect(g1.children).toHaveLength(1);
      expect(g1.children[0]).toMatchObject({
        field: 'age',
        operator: 'gt',
        value: 18,
      });
  ...
  it('respects custom idGenerator in options', () => {
    ...
    expect(result.children[0]).toMatchObject({ id: customId });
  });
...
describe('updateRule', () => {
  ...
  it('preserves immutability', () => {
    const result = updateRule(filter, 'r1', { field: 'email' });
    expect(result).not.toBe(filter);
    expect(filter.children[0]).toMatchObject({ field: 'name' });
    expect(result.children[0]).toMatchObject({ field: 'email' });
  });
  ...
  it('updates rule inside nested group', () => {
    ...
    const g1 = result.children[0];
    if ('children' in g1) {
      expect(g1.children[0]).toMatchObject({ id: 'r3', value: 'inactive' });
    }
  });
  ...
  it('updates root-level rule when nested groups exist', () => {
    ...
    expect(result.children[1]).toMatchObject({ id: 'r1', value: 'Jane' });
  });
});
...
describe('removeRule', () => {
  ...
  it('removes rule from nested group', () => {
    ...
    if ('children' in g1) {
      expect(g1.children).toHaveLength(1);
      expect(g1.children[0]).toMatchObject({ id: 'r2' });
    }
  });
...
describe('moveRule', () => {
  it('moves rule within same group (reorder)', () => {
    ...
    expect(result.children[0]).toMatchObject({ id: 'r3' });
    expect(result.children[1]).toMatchObject({ id: 'r1' });
    expect(result.children[2]).toMatchObject({ id: 'r2' });
  });
  ...
  it('moves rule between different groups', () => {
    ...
    const g1 = result.children[0];
    const g2 = result.children[1];
    expect(g1).toHaveProperty('children');
    expect(g2).toHaveProperty('children');
    if ('children' in g1 && 'children' in g2) {
      expect(g1.children).toHaveLength(0);
      expect(g2.children).toHaveLength(2);
      expect(g2.children[0]).toMatchObject({ id: 'r2' });
      expect(g2.children[1]).toMatchObject({ id: 'r1' });
    }
  });
  ...
  it('moves rule from descendant group to ancestor group without duplicates', () => {
    ...
    if ('children' in g1) {
      ...
      const g2 = g1.children[1];
      expect(g2).toHaveProperty('children');
      if (g2 && 'children' in g2) {
  ...
  it('moves rule between sibling groups at same depth without duplicates', () => {
    ...
    const g1 = result.children[0] as Filter;
    const g2 = result.children[1] as Filter;
    expect(g1.children).toHaveLength(1);
    expect((g1.children[0] as FilterRule).id).toBe('r1');
    expect(g2.children).toHaveLength(2);
    expect((g2.children[0] as FilterRule).id).toBe('r2');
    expect((g2.children[1] as FilterRule).id).toBe('r3');
  ...
  it('same-group move: position is post-removal index', () => {
    ...
    expect((result.children[0] as FilterRule).id).toBe('r2');
    expect((result.children[1] as FilterRule).id).toBe('r1');
    expect((result.children[2] as FilterRule).id).toBe('r3');
  });
  ...
  it('moves root-level rule to nested group without duplicates', () => {
    ...
    expect(result.children).toHaveLength(1);
    expect((result.children[0] as FilterGroup).id).toBe('g1');
    const g1 = result.children[0] as Filter;
    expect(g1.children).toHaveLength(1);
    expect((g1.children[0] as FilterRule).id).toBe('r1');
...
describe('addGroup', () => {
  it('adds empty group to root', () => {
    ...
    expect(added).toHaveProperty('children');
