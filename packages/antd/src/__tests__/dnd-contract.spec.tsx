...
function renderWithDnd(onChange = jest.fn()) {
  const props: React.ComponentProps<typeof AntdFilterBuilder> & { dnd: boolean } = {
    schema,
    value: filter,
    onChange,
    dnd: true,
  ...
  render(<AntdFilterBuilder {...props} />);
  return onChange;
}
...
test('AntdFilterBuilder moves a rule down through the DnD adapter boundary', () => {
  const onChange = renderWithDnd();
...
test('AntdFilterBuilder moves a child group through the keyboard fallback', () => {
...
test('AntdFilterBuilder omits keyboard move controls unless dnd is enabled', () => {
  ...
  rerender(<AntdFilterBuilder {...props} />);

  expect(screen.queryByRole('button', { name: /move r2 up/i })).toBeNull();
});

test('AntdFilterBuilder disables invalid edge moves', () => {
  renderWithDnd();
